...
const Trade: React.FC = () => {
  ...
  const canAffordTrade = useCallback(() => {
    ...
    if (tradeType === TransactionType.BUY) {
      ...
      const position = userPortfolio.positions.find((p: Position) => p.stockId === selectedStock.id);
  ...
  return (
    <div className="container mx-auto px-4 py-6 space-y-6" role="main">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="p-6 bg-white rounded-lg shadow-md dark:bg-gray-800">
          {selectedStock ? (
            ...
            <div className="flex items-center justify-center h-full">
              <span className="text-gray-500 dark:text-gray-400">Select a stock to view details</span>
            </div>
          )}
        </div>
        ...
        <div className="p-6 bg-white rounded-lg shadow-md dark:bg-gray-800">
          {userPortfolio && (
            <div className="mb-6 border-b border-gray-200 pb-4" role="region" aria-label="Account information">
              <div className="flex justify-between items-center mb-2">
                <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Available Cash</div>
                <div className="font-bold text-gray-800 dark:text-gray-200" aria-live="polite">{formatCurrency(userPortfolio.cash)}</div>
              </div>
              <div className="flex justify-between items-center">
                <div className="text-sm font-medium text-gray-500 dark:text-gray-400">Portfolio Value</div>
                <div className="font-bold text-gray-800 dark:text-gray-200" aria-live="polite">{formatCurrency(userPortfolio.totalValue)}</div>
              </div>
            </div>
          )}

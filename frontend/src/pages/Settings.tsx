  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-gray-600 dark:text-gray-400 max-w-3xl">
        Customize your game experience. These settings focus on visual cues and performance
        optimizations for playing in real-time with thousands of other players.
      </p>
          ...
          <div key={category.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            ...
            <div className="space-y-6">
              {category.settings.map(setting => (
                <div
                  key={setting.id}
                  className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-4 border-b border-gray-200 dark:border-gray-700 last:border-0 last:pb-0"
                >
                  ...
                  <div className="md:w-5/12">
                      ...
                      <div className="flex items-center justify-end">
                        <button
                          ...
                          onClick={() =>
                            handleSettingChange(
                              setting.id,
                              !settings[setting.id as keyof UserSettings]
                            )
                          }
                          role="switch"
                        ...
                        >
                          <span className="sr-only">
                            {settings[setting.id as keyof UserSettings] ? 'On' : 'Off'}
                          </span>
                          <span
                        ...
                        <span id={`label-${setting.id}`} className="sr-only">
                          {setting.label}
                        </span>
                      </div>
                    )}
                    ...
                    {setting.type === 'select' && (
                      <select
                        id={setting.id}
                        value={String(
                          settings[setting.id as keyof UserSettings] ?? setting.defaultValue
                        )}

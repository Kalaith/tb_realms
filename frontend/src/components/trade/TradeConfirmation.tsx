...
const TradeConfirmation: React.FC<TradeConfirmationProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" role="dialog" aria-modal="true" aria-labelledby="confirmation-title">
      <div 
        className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md mx-4"
        ref={modalRef}
        tabIndex={-1}
      >
        <h2 id="confirmation-title" className="text-xl font-bold text-gray-900 dark:text-white mb-4">
          Confirm {tradeType === TransactionType.BUY ? 'Purchase' : 'Sale'}
        </h2>
        
        <div className="space-y-3 mb-6">
          <p className="text-gray-700 dark:text-gray-300">
            <span className="font-medium text-gray-900 dark:text-white">Stock:</span> {stock.name} ({stock.symbol})
          </p>
          <p className="text-gray-700 dark:text-gray-300">
            <span className="font-medium text-gray-900 dark:text-white">Action:</span> {tradeType === TransactionType.BUY ? 'Buy' : 'Sell'} {shares} share{shares !== 1 ? 's' : ''}
          </p>
          <p className="text-gray-700 dark:text-gray-300">
            <span className="font-medium text-gray-900 dark:text-white">Price:</span> {formatCurrency(stock.currentPrice)} per share
          </p>
          <p className="text-gray-700 dark:text-gray-300">
            <span className="font-medium text-gray-900 dark:text-white">Total:</span> {formatCurrency(amount)}
          </p>
        </div>

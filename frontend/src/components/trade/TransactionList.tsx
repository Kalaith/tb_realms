...
const TransactionList: React.FC<TransactionListProps> = ({ transactions }) => {
  return (
    <div className="w-full" role="region" aria-labelledby="recent-transactions-title">
      <h3 id="recent-transactions-title" className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">Recent Transactions</h3>
      ...
      {transactions.length > 0 ? (
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">{transaction.shares} share(s) @ {formatCurrency(transaction.price)}</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-medium text-gray-900 dark:text-white">{formatCurrency(transaction.total)}</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">{formatDateTime(transaction.timestamp)}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="p-4 text-center text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 rounded-lg" role="status">

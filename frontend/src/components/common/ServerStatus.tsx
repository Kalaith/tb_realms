...
const ServerStatus: React.FC<ServerStatusProps> = ({ onStatusChange }) => {
  ...
  return (
    <div className={`fixed bottom-4 right-4 p-4 rounded-lg shadow-lg z-50 ${isConnected ? 'bg-green-100' : 'bg-red-100'}`}>
      <div className="flex items-center">
        <div className={`w-3 h-3 rounded-full mr-2 ${isChecking ? 'bg-yellow-500' : (isConnected ? 'bg-green-500' : 'bg-red-500')}`}></div>

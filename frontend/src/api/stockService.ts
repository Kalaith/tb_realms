  ...
  async getByFilters(filters: StockFilters): Promise<ApiResponse<Stock[]>> {
    try {
      ...
      if (filters.marketCapMin !== undefined)
        queryParams.append("marketCapMin", filters.marketCapMin.toString());
      if (filters.marketCapMax !== undefined)
        queryParams.append("marketCapMax", filters.marketCapMax.toString());
      ...
      const endpoint = queryString
        ? `${this.endpoint}/filter?${queryString}`
  ...
  async getPriceHistory(
    ...
    try {
      const response = await apiClient.get<unknown>(
        `${this.endpoint}/${stockId}/prices?timeFrame=${timeFrame}`,
      );
